...
!function(e) {
    "function" == typeof define && define.amd ? define('vendor/jquery.nicescroll.min', ["jquery"], e) : "object" == typeof exports ? module.exports = e(require("jquery")) : e(jQuery)
}(function(e) {
    "use strict";
    var o = !1
      , t = !1
...
define('app/index', ['./common', 'vendor/jquery.nicescroll.min', 'helper'], function() {
    if ($(window).width() < 992) {
        $("#my-video[autoplay][muted]").remove();
    }
    ...
    // banner swiper 左右箭头增加ga事件
    setTimeout(function() {
        $(".index-banner .swiper-button-prev").get(0).setAttribute("onclick", "ga('send', 'event', 'corp_hp_clicks', 'banner_switch_left')");
        $(".index-banner .swiper-button-next").get(0).setAttribute("onclick", "ga('send', 'event', 'corp_hp_clicks', 'banner_switch_right')");
    }, 300);

    (function indexNicescrollFixed() {
    ...
    function setNiceScroll() {
        var colHeight = $('.col-item').eq(0).height();
        $('.news-item .list-box').height(colHeight - 20);
        var ns = $("#wrapper").niceScroll({
            cursorcolor: "#adadad",
            cursorwidth: "6px",
            cursorborderradius: "5px",
            // 滚动条圆角（像素）
        ...
        ns.scrollend(function(info) {
            if (info.current.y >= (ns.page.maxh || 0) - 30) {
                ns.me.addClass("ended");
            } else {
                ns.me.removeClass("ended");
            }
        });
        ...
        $(window).resize(function() {
            colHeight = $('.col-item').eq(0).height();
            $('.news-item .list-box').height(colHeight - 20);
            setTimeout(function() {
                // 打开文件夹1s后重置滚动条
    ...
    if (wWidth > 992 && $("#wrapper").length) {
        $('.col-item').imagesLoaded().always(function() {
            setTimeout(function() {
                setNiceScroll();
            }, 100);
        });
    ...
    $(document).on("videojs-resolution-switcher-loaded", function(e) {
        ...
        if ($("#my-video").length == 0)
            return;
        var playFn = function() {
            var p = videojs.getPlayers()["my-video"];
            if (p) {
                p.play();
            }
        };
        ...
        io.observe($("#my-video").parent().get(0));
    });
});
